...
> 巩固已有知识，填补遗漏。**预计 3-5 天。**

- [ ] **1.1 复习基础类型与运算符**
    - [ ] 位运算符 (`&`, `|`, `^`, `&^`, `<<`, `>>`) — 分布式系统中 bitmask 常见
    - [ ] 类型别名 vs 类型定义 (`type A = B` vs `type A B`)
    - [ ] `iota` 枚举 — 状态码、协议常量常用

- [ ] **1.2 复习控制结构**
    - [ ] `goto` 的适用场景（错误统一处理）
    - [ ] `for range` 中修改变量不生效的问题（值拷贝陷阱）

- [ ] **1.3 复习函数与闭包**
    - [ ] 函数作为第一类值：赋值、传参、返回
    - [ ] 闭包陷阱：循环变量捕获（Go 1.22 已修复但仍需理解原理）
    - [ ] defer 的参数求值时机（立即求值 vs 延迟执行）

- [ ] **1.4 复习数组与切片**
    - [ ] 切片底层原理：ptr/len/cap 三元组
    - [ ] append 扩容机制 (capacity doubling)
    - [ ] 切片共享底层数组的坑（sub-slice 修改影响原数据）
    - [ ] 练习：实现一个简单的 `RingBuffer`（环形缓冲区 — 分布式消息队列基础）

- [ ] **1.5 复习 Map**
    - [ ] map 非并发安全，并发读写会 panic
    - [ ] map 遍历顺序随机（故意设计）
    - [ ] 练习：统计一段文本的词频，按频率排序输出
